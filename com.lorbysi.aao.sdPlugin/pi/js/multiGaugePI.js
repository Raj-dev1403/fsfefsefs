function MultiGaugePI(inContext, inStreamDeckVersion, inPluginVersion) {
    var instance = this;
    ...
    var cnt = "\
				...
				<div>\
                    ...
                    <div class='sdpi-item'> \
						<div class='sdpi-item-label'>Layer</div> \
						<input type='number' id='textlayer' value='0' />\
					</div> \
                    <div class='sdpi-item'> \
						<div class='sdpi-item-label'></div> \
						<button class='sdpi-item-value max30' id='layerTextUpbtn'>Up</button> \
						<button class='sdpi-item-value max30' id='layerTextDownbtn'>Down</button> \
					</div> \
					<div class='sdpi-item'> \
						<div class='sdpi-item-label'>Visibility variable</div> \
					...
					<div class='sdpi-item'> \
						...
						<select class='sdpi-item-value select' id='cbTextStretch'> \
							<option value='1'>Yes&nbsp;&nbsp;</option> \
							<option value='0'>No</option> \
						</select> \
						<label><span></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label> \
    ...
    document.getElementById('layerTextUpbtn').addEventListener('click', layerUpTextDefinition);
	document.getElementById('layerTextDownbtn').addEventListener('click', layerDownTextDefinition);
	...
	if (settings["textdefs"]){
        ...
        document.getElementById('textlayer').value = '0';
	...
	function addTextDefinition(inEvent) {
        ...
        document.getElementById('textlayer').value = document.getElementById('textdefs').options.length;
		resetTextFields();
	}
	...
	function saveTextDefinition(inEvent) {
		if (currentTextDef){
			...
			if (headerstr && headerstr.length > 0){
				...
				currentTextDef.splitstretch = document.getElementById('cbTextStretch').value;
				if (document.getElementById('rdio1').checked)
					currentTextDef.align = 1;
	...
	function resetTextFields(){
		...
		document.getElementById('cbTextStretch').value = '0';
	}
	...
	function loadTextDefValues(currentTextDef){
		if (currentTextDef){
            ...
            if (selectedTextDefIndex >= 0){
				document.getElementById('textlayer').value = selectedTextDefIndex;
			} else {
				document.getElementById('textlayer').value = document.getElementById('textdefs').options.length;
			}
			...
			if (colStr && colStr.length > 0){
				document.getElementById('cbTextStretch').value = colStr;
			} else {
			  document.getElementById('cbTextStretch').value = '0';
			}
			document.getElementById('rdio2').checked = true;
    	...
    	function layerUpTextDefinition(inEvent){
		if (currentImageDef && selectedTextDefIndex >= 0 && selectedTextDefIndex < document.getElementById('textdefs').options.length - 1){
			var tmpObj = settings["textdefs"][parseInt(selectedTextDefIndex)];
			var upObj = settings["textdefs"][(parseInt(selectedTextDefIndex) + 1)];
			settings["textdefs"][selectedTextDefIndex] = upObj;
			selectedTextDefIndex++;
			settings["textdefs"][selectedTextDefIndex] = tmpObj;
			document.getElementById('submitbtn').click();
			var textSelect = document.getElementById('textdefs');
			while (textSelect.options.length > 0) {
				textSelect.remove(0);
			}   
			for (let i = 0; i < settings["textdefs"].length; i++){
				textSelect.options[textSelect.options.length] = new Option(settings["textdefs"][i].header, i);
	...
	function layerDownTextDefinition(inEvent){
		if (currentImageDef && selectedTextDefIndex > 0){
			var tmpObj = settings["textdefs"][parseInt(selectedTextDefIndex)];
			var dnObj = settings["textdefs"][(parseInt(selectedTextDefIndex) - 1)];
			settings["textdefs"][selectedTextDefIndex] = dnObj;
			selectedTextDefIndex--;
			settings["textdefs"][selectedTextDefIndex] = tmpObj;
			document.getElementById('submitbtn').click();
			var textSelect = document.getElementById('textdefs');
