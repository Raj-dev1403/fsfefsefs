function OnOffPI(inContext, inStreamDeckVersion, inPluginVersion) {
        var instance = this;
    PI.call(this, inContext, inStreamDeckVersion, inPluginVersion);

    var cnt = "<div class='sdpi-item'> \
					<div class='sdpi-item-label'>Write event</div> \
    ...
    function valuesChanged(inEvent) {
	   ...
	   vvarStr = document.getElementById('longsimevt').value;
		...
		vvarStr = document.getElementById('autoscript').value;
		if (vvarStr && vvarStr.length > 0){
			settings["updateAction"] = document.getElementById('autoscript').value;
