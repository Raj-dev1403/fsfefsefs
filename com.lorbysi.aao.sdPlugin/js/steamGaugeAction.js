function SteamGaugeAction(inContext, inSettings, coordinates, canvs) {
    // Init Action
    var instance = this;
    var cnv = canvs;
	var coords = "99_99";
    ...
    function loadImages() {
        ...
        promises.push(loadImage(turnImage));
        ...
        turnImage.src = (settings["turnimg"] && settings["turnimg"].length > 0) ? settings["turnimg"] : '';
    ...
    var lastDrawnRotation = null;
    var lastDrawnOutStr = null;
    
    function rotationChanged(oldValue, newValue) {
        ...
        return Math.abs(oldValue - newValue) >= 0.0174533;
    }
    ...
    this.drawButton = function() {
        ...
        var rotation = (settings["turnimg"] && settings["turnimg"].length > 0) ? (simVal * settings["multiplier"]) - degrees_to_radians(settings["turnoffset"]) : null;
		var outStr = null;
        ...
        if (!pimode && outStr === lastDrawnOutStr && !rotationChanged(lastDrawnRotation, rotation)) {
            return false;
        }
        ...
        if (rotation !== null){
			...
			ctx.translate(settings["xshift"], settings["yshift"]);
			ctx.rotate(rotation);
			ctx.translate(-settings["xshift"], -settings["yshift"]);
			ctx.drawImage(turnImage, 0, 0, 144, 144);
			ctx.restore();
		}
		...
		if (outStr !== null){
			...
			if (widthPx > 144){
				...
				ctx.font = Math.floor(factor) + 'px ' + settings["font"];
			}
